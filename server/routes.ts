...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.get("/time-slots", async (req, res) => {
    try {
      ...
      if (timeSlots.length === 0) {
        ...
        const newTimeSlots = await storage.getTimeSlots(barberId, date);
  ...
  apiRouter.get("/appointments", authenticate, async (req, res) => {
    try {
      ...
      if (req.user!.role === 'client') {
        ...
        appointments = await storage.getAppointmentsByBarber(barber.id);
      } else {
      ...
      const enhancedAppointments = await Promise.all(appointments.map(async (appointment) => {
        ...
        const barberUser = barber.userId ? await storage.getUser(barber.userId) : null;
  ...
  apiRouter.post("/appointments", authenticate, async (req, res) => {
    try {
      ...
      const barber = await storage.getBarber(appointmentData.barberId);
      if (!barber) {
        return res.status(404).json({ message: "Barber not found" });
      }
  ...
  apiRouter.patch("/appointments/:id", authenticate, async (req, res) => {
    try {
      ...
      // Get user, barber, and service details for the response
      const [user, barber, service] = await Promise.all([
        storage.getUser(updatedAppointment.userId),
        storage.getBarber(updatedAppointment.barberId),
        storage.getService(updatedAppointment.serviceId)
      ]);
